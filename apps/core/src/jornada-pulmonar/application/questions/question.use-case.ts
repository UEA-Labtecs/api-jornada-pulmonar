...
import type { QuestionRepository as QuestionRepositoryType } from './question-repository';

@Injectable()
export class QuestionsUseCase implements IQuestionsUseCase {
  constructor(
    private readonly QuestionRepository: QuestionRepositoryType,
  ...
  async createQuestinModule(data: any) {
    ...
    if (payload.moduleId) {
      ...
      const modules = await this.ModuleRepository.findAll({
        title: payload.titleUnit,
      });

      if (modules.length > 0) {
        // Se módulos existirem, usa o ID do primeiro módulo encontrado
        console.log(
          '⚠️ Encontrado módulo existente por título:',
          modules[0].id,
        );
        ...
        const newModule = await this.ModuleRepository.create(
          new Modules({ title: payload.titleUnit, userId: payload.userId }),
        );
    ...
    await Promise.all(
      payload.alternatives.map(async (alternative) => {
        const ops = await this.OptionRepository.create(
          new Options({
        ...
        if (alternative.correctAlternative === true) {
          await this.ResponseRepository.create(
            new Responses({ choiceId: ops.id, questionId: question.id }),
          );
        }
  ...
  async updateQuestion(id: string, data: any) {
    ...
    if (payload.moduleId) {
      console.log(
        '✅ Usando unidade existente (update), ID:',
        payload.moduleId,
      );
      moduleId = payload.moduleId;
    } else {
  ...
  async findAllQuestion(query: any) {
    ...
    const correctResponses = await this.ResponseRepository.findAll({
    ...
    questions.forEach((question) => {
      ...
      question.alternatives.forEach((alternative) => {
        const correctResponse = correctResponses.find(
          (response) => response.choiceId === alternative.id,
        );
