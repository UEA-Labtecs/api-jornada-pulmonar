...
export const makeVerifyUserController = (opitionId: string, questionId: string, userId: string, time: number) => {
  ...
  const useCase = new UsersUseCase(repositoryUser, repositoryQuestion, userResponsesRepository);
  return new VerifyResponseUserController(useCase, opitionId, questionId, userId, time);
};
